...
contract DigitalAssetNFT is ERC721,AccessControl{
    ...
    function mintAsset(string memory _name,string memory _metadataURI,string memory _assetType,bool _isActive) external {
        require(accessControl.hasPermission(msg.sender, Permission.CREATE_ASSET), "You don't have permission to mint assets");
        _safeMint(OWNER,tokenID);
        assets[tokenID]=Asset({tokenId:tokenID,name:_name,ownedBy:msg.sender,metadataURI:_metadataURI,assetType:_assetType,assetState:AssetState.CREATED,isActive:_isActive,createdBy:msg.sender,did:"",assigningAssetTo:address(0)});
    ...
    function getAsset(uint256 _tokenID) external view returns(uint256 ,string memory, string memory ,string memory, bool ,address ){
        return (assets[_tokenID].tokenId,assets[_tokenID].name,assets[_tokenID].metadataURI,assets[_tokenID].assetType,assets[_tokenID].isActive,assets[_tokenID].createdBy);
    }
